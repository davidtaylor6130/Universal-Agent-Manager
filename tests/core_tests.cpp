...
UAM_TEST(GeminiPlanPermissionPromotesMarkdownToPlanBlock)
{
	TempDir temp("uam-gemini-plan-permission");
	uam::AppState app;
	app.data_root = temp.root;
	...
	const std::string plan_markdown = "# Plan\n\n1. Inspect files\n2. Patch runtime";
	UAM_ASSERT(uam::ProcessAcpLineForTests(app, *raw_session, app.chats.front(), nlohmann::json{
		...
		{"params", {
			...
			{"options", nlohmann::json::array({
				...
				{{"optionId", "plan"}, {"name", "Keep planning"}, {"kind", "reject"}},
			})},
		}},
	}.dump()));

	UAM_ASSERT_EQ(raw_session->plan_summary, plan_markdown);
	...
	UAM_ASSERT_EQ(raw_session->pending_permission.kind, std::string("switch_mode"));
	UAM_ASSERT_EQ(raw_session->turn_events.size(), static_cast<std::size_t>(1));
	UAM_ASSERT_EQ(raw_session->turn_events[0].type, std::string("plan"));
	UAM_ASSERT_EQ(app.chats.front().messages.size(), static_cast<std::size_t>(2));
	UAM_ASSERT_EQ(app.chats.front().messages[1].plan_summary, plan_markdown);
	UAM_ASSERT_EQ(app.chats.front().messages[1].blocks.size(), static_cast<std::size_t>(1));
	UAM_ASSERT_EQ(app.chats.front().messages[1].blocks[0].type, std::string("plan"));

	const nlohmann::json serialized = uam::StateSerializer::Serialize(app);
	const nlohmann::json acp = serialized["chats"][0]["acpSession"];
	UAM_ASSERT_EQ(acp.value("planSummary", ""), plan_markdown);
	UAM_ASSERT_EQ(acp["pendingPermission"].value("requestId", ""), std::string("7"));
	UAM_ASSERT_EQ(serialized["chats"][0]["messages"][1].value("planSummary", ""), plan_markdown);
	UAM_ASSERT_EQ(serialized["chats"][0]["messages"][1]["blocks"][0].value("type", ""), std::string("plan"));
}

UAM_TEST(GeminiNonPlanPermissionDoesNotPromoteToPlan)
{
	TempDir temp("uam-gemini-non-plan-permission");
	uam::AppState app;
	app.data_root = temp.root;
	...
	UAM_ASSERT(uam::ProcessAcpLineForTests(app, *raw_session, app.chats.front(), R"({"jsonrpc":"2.0","id":8,"method":"session/request_permission","params":{"toolCall":{"toolCallId":"tool-1","title":"Read file","kind":"read","status":"pending","content":{"type":"text","text":"Read /tmp/project/file.txt"}},"options":[{"optionId":"allow-once","name":"Allow once","kind":"allow_once"}]}})"));

	UAM_ASSERT_EQ(raw_session->plan_summary, std::string(""));
	UAM_ASSERT_EQ(raw_session->plan_entries.size(), static_cast<std::size_t>(0));
	UAM_ASSERT_EQ(raw_session->pending_permission.request_id_json, std::string("8"));
	UAM_ASSERT_EQ(raw_session->turn_events.size(), static_cast<std::size_t>(2));
	UAM_ASSERT_EQ(raw_session->turn_events[0].type, std::string("tool_call"));
	UAM_ASSERT_EQ(raw_session->turn_events[1].type, std::string("permission_request"));
