...
namespace ollama_engine::internal::vectorised_rag
{
	namespace
	{
		...
		bool EnsureRemoteClone(const std::string& pSRemoteUrl, const std::string& pSBranch, const fs::path& pPathClone, std::string* pSErrorOut)
		{
		...
		bool ResolveSourceSpec(Context& pContext, const std::optional<std::string>& pOptSVectorFile, SourceSpec* pPtrSourceSpecOut, std::string* pSErrorOut)
		{
		...
		std::vector<ChunkRecord> BuildFileSummaryFallback(const std::string& pSSourceId, const std::string& pSRelativePath, const std::string& pSContent, const std::size_t piChunkCharLimit)
		{
			std::vector<ChunkRecord> lVecChunks;
			ChunkRecord lChunk;
		...
		std::vector<ChunkRecord> ExtractChunksFromDocumentationFile(const std::string& pSSourceId, const std::string& pSRelativePath, const std::string& pSContent, const std::size_t piChunkCharLimit)
		{
		...
		std::vector<ChunkRecord> ExtractChunksFromCppFile(const std::string& pSSourceId, const std::string& pSRelativePath, const std::string& pSContent, const std::size_t piChunkCharLimit)
		{
		...
		std::optional<std::vector<float>> TryEmbedWithServer(Context& pContext, const RuntimeOptions& pRuntimeOptions, const fs::path& pPathModel, const std::string& pSText)
		{
		...
		bool EnsureDirectEmbeddingRuntime(Context& pContext, const fs::path& pPathModel, const RuntimeOptions& pRuntimeOptions, std::string* pSErrorOut)
		{
			if (pPathModel.empty())
			{
				if (pSErrorOut != nullptr)
		...
		std::optional<std::vector<float>> BuildEmbeddingDirect(Context& pContext, const RuntimeOptions& pRuntimeOptions, const std::string& pSText, std::string* pSErrorOut)
		{
		...
		std::optional<std::vector<float>> BuildEmbedding(Context& pContext, const RuntimeOptions& pRuntimeOptions, const std::string& pSText, std::string* pSErrorOut)
		{
		...
		bool ResolveDatabaseInput(const std::string& pSDatabaseInput, const RuntimeOptions& pRuntimeOptions, std::vector<fs::path>* pPtrVecPathDatabasesOut, std::string* pSErrorOut)
		{
		...
		bool OpenDatabaseForSource(const std::string& pSSourceId, const RuntimeOptions& pRuntimeOptions, sqlite3** pPtrDatabaseOut, fs::path* pPathDatabaseOut)
		{
...
CREATE INDEX IF NOT EXISTS idx_chunks_source_file ON chunks(source_id, file_path);
		...
		bool SelectFileHash(sqlite3* pPtrDatabase, const std::string& pSSourceId, const std::string& pSFilePath, std::string* pPtrHashOut)
		{
		...
		bool UpsertFileRow(sqlite3* pPtrDatabase, const std::string& pSSourceId, const std::string& pSFilePath, const std::string& pSContentHash, const std::uint64_t piMtimeTicks, const std::uintmax_t piFileSize)
		{
		...
		std::string FormatSnippet(const std::string& pSSourceId, const std::string& pSFilePath, const int piStartLine, const int piEndLine, const std::string& pSChunkType, const std::string& pSSymbolName, const std::string& pSParentSymbol, std::string pSRawText)
		{
