...
namespace ollama_engine::internal
{
	...
	class LocalOllamaCppEngine final : public EngineInterface
	{
	  public:
		...
		/// <inheritdoc />
		bool LoadRagDatabases(const std::vector<std::string>& pVecSDatabaseInputs, std::string* pSErrorOut = nullptr) override;
		/// <inheritdoc />
		std::vector<std::string> Fetch_Relevant_Info(const std::string& pSPrompt, std::size_t piMax, std::size_t piMin) override;
		/// <inheritdoc />
