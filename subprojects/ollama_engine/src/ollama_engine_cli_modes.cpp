...
void TryAutoApplyFinalProfile(ollama_engine_cli::common::CliSession* pCliSessionInOut,
                              const bool pbPrintNoMatchMessage) {
  if (pCliSessionInOut == nullptr || !pCliSessionInOut->pPtrEngine) {
    return;
  }

  const std::filesystem::path lPathFinalProfileDirectory =
      ollama_engine_cli::autotune::GetAutoTuneFinalProfileDirectory(pCliSessionInOut->pPathModelFolder);
  const std::vector<ollama_engine_cli::autotune::AutoTuneTemplate> lVecAutoTuneFinalProfiles =
      ollama_engine_cli::autotune::LoadAutoTuneFinalProfiles(lPathFinalProfileDirectory);
  const std::string lSHardwareFingerprint = ollama_engine_cli::autotune::BuildHardwareFingerprint();
  const std::optional<ollama_engine_cli::autotune::AutoTuneTemplate> lOptAutoProfile =
      ollama_engine_cli::autotune::FindBestMatchingTemplate(lVecAutoTuneFinalProfiles,
                                                            pCliSessionInOut->pSLoadedModelName,
                                                            lSHardwareFingerprint);
  ...
  std::string lSError;
  if (ollama_engine_cli::autotune::ApplyAutoTuneTemplate(pCliSessionInOut->pPtrEngine.get(), *lOptAutoProfile,
                                                          &pCliSessionInOut->pSLoadedModelName, &lSError)) {
    std::cout << "autotune> auto-applied finalized profile '" << lOptAutoProfile->pSName << "'\n";
    std::cout << "autotune> "
              << ollama_engine_cli::autotune::DescribeGenerationSettings(
                     pCliSessionInOut->pPtrEngine->GetGenerationSettings())
              << "\n";
    return;
  }
...
std::optional<std::filesystem::path> ResolveCustomTestsDirectory(int argc, char** argv) {
  ...
  const std::string lSInput =
      ollama_engine_cli::common::Trim(ollama_engine_cli::common::ReadLine("tests> path to Question_N test files (/quit to exit): "));
...
int RunFinetuneWizardCli(int argc, char** argv) {
  ...
  autotune::RunAutoTuneWizard(lCliSession.pPtrEngine.get(), lCliSession.pVecSModels, lCliSession.pPathModelFolder,
                              &lCliSession.pSLoadedModelName);
  return 0;
}
