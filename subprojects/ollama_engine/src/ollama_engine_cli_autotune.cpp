...
bool SaveAutoTuneTemplateXmlFile(const ollama_engine_cli::autotune::AutoTuneTemplate& pAutoTuneTemplate,
...
bool SaveAutoTuneFinalProfileOleFile(const ollama_engine_cli::autotune::AutoTuneTemplate& pAutoTuneTemplate,
                                     const std::filesystem::path& pPathDirectory, std::string* pSErrorOut) {
  std::error_code lErrorCode;
  std::filesystem::create_directories(pPathDirectory, lErrorCode);
  if (lErrorCode) {
...
std::optional<ollama_engine_cli::autotune::AutoTuneTemplate> LoadAutoTuneTemplateXmlFile(
    const std::filesystem::path& pPathTemplateFile) {
...
std::optional<ollama_engine_cli::autotune::AutoTuneTemplate> LoadAutoTuneFinalProfileOleFile(
    const std::filesystem::path& pPathTemplateFile) {
  std::ifstream lFileIn(pPathTemplateFile, std::ios::in);
  if (!lFileIn.good()) {
    return std::nullopt;
...
AutoTuneEvaluationSummary EvaluateAutoTuneSettings(ollama_engine::EngineInterface* pPtrEngine,
  ...
  if (pAutoTuneWizardConfig.pAutoTuneEvaluationMode == AutoTuneEvaluationMode::BuiltInBenchmarks) {
    const std::vector<ollama_engine_cli::tests::BenchmarkProbe> lVecBenchmarkProbes =
        ollama_engine_cli::tests::BuildStandardOpenSourceBenchmarkProbes();
    ...
    for (int liRun = 0; liRun < pAutoTuneWizardConfig.piRunsPerSetting; ++liRun) {
      for (const ollama_engine_cli::tests::BenchmarkProbe& lBenchmarkProbe : lVecBenchmarkProbes) {
        const ollama_engine_cli::common::PromptRunResult lPromptRunResult =
            ollama_engine_cli::common::RunPromptWithMetrics(pPtrEngine, lBenchmarkProbe.pSPrompt);
        ++liCaseCount;
        ...
        if (lPromptRunResult.pSendMessageResponse.pbOk &&
            ollama_engine_cli::tests::IsBenchmarkProbePass(lBenchmarkProbe, lPromptRunResult.pSendMessageResponse.pSText)) {
          ++liPassCount;
        }
      }
    }
  } else {
    const std::vector<ollama_engine_cli::tests::QuestionAnswerCase> lVecQuestionAnswerCases =
        ollama_engine_cli::tests::LoadQuestionAnswerCases(pAutoTuneWizardConfig.pPathQuestionAnswerFolder);
    ...
    for (int liRun = 0; liRun < pAutoTuneWizardConfig.piRunsPerSetting; ++liRun) {
      for (const ollama_engine_cli::tests::QuestionAnswerCase& lQuestionAnswerCase : lVecQuestionAnswerCases) {
        const ollama_engine_cli::common::PromptRunResult lPromptRunResult =
            ollama_engine_cli::common::RunPromptWithMetrics(pPtrEngine, lQuestionAnswerCase.pSQuestionText);
        ++liCaseCount;
        ldTotalMs += lPromptRunResult.pdTotalMilliseconds;
        ldTotalTtftMs += lPromptRunResult.pdTimeToFirstTokenMilliseconds;
        ldTotalTokPerSec += lPromptRunResult.pdTokensPerSecond;

        const std::string lSActual =
            ollama_engine_cli::common::NormalizeForMatch(lPromptRunResult.pSendMessageResponse.pSText);
...
void RunAutoTuneWizard(ollama_engine::EngineInterface* pPtrEngine, const std::vector<std::string>& pVecSModels,
  ...
  lAutoTuneWizardConfig.pSModelName =
      (pSLoadedModelNameInOut != nullptr) ? *pSLoadedModelNameInOut : pVecSModels.front();
  ...
  const std::string lSModelSelectionInput =
      ollama_engine_cli::common::ReadLine("autotune> pick model (number/name, Enter=current): ");
  if (!ollama_engine_cli::common::Trim(lSModelSelectionInput).empty()) {
    const std::optional<std::string> lOptSelectedModel =
        ollama_engine_cli::common::ResolveSelectedModel(ollama_engine_cli::common::Trim(lSModelSelectionInput),
                                                         pVecSModels);
  ...
  const int liEvaluationChoice =
      std::clamp(ParseIntOrDefault(ollama_engine_cli::common::ReadLine("autotune> choose 1 or 2 [1]: "), 1), 1, 2);
  if (liEvaluationChoice == 2) {
    lAutoTuneWizardConfig.pAutoTuneEvaluationMode = AutoTuneEvaluationMode::QuestionAnswerFolder;
    const std::string lSQuestionFolderPath =
        ollama_engine_cli::common::Trim(ollama_engine_cli::common::ReadLine("autotune> path to Question_N files: "));
  ...
  lAutoTuneWizardConfig.pdTargetTokensPerSecond = std::max(
      0.0, ParseDoubleOrDefault(ollama_engine_cli::common::ReadLine("autotune> target speed tok/s (0=ignore) [0]: "), 0.0));
  lAutoTuneWizardConfig.pdTargetLatencyMilliseconds = std::max(
      0.0,
      ParseDoubleOrDefault(ollama_engine_cli::common::ReadLine("autotune> target latency ms (0=ignore) [0]: "), 0.0));
  lAutoTuneWizardConfig.piRunsPerSetting =
      std::clamp(ParseIntOrDefault(ollama_engine_cli::common::ReadLine("autotune> runs per setting for average [3]: "), 3),
                 1, 10);
  ...
  std::string lSTemplateName =
      ollama_engine_cli::common::Trim(ollama_engine_cli::common::ReadLine("autotune> profile/template base name [" +
                                                                           lSDefaultTemplateName + "]: "));
