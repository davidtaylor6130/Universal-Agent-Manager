...
bool IsCommentLine(const std::string& trimmed_line) {
  return trimmed_line.rfind("#", 0) == 0 ||
         trimmed_line.rfind(";", 0) == 0 ||
         trimmed_line.rfind("//", 0) == 0;
}

FrontendAction MakeDefaultAction(const std::string& key,
...
void AssignField(FrontendAction& action,
...
void NormalizeFrontendActionMap(FrontendActionMap& action_map) {
  ...
  std::sort(normalized.begin(), normalized.end(), [](const FrontendAction& a, const FrontendAction& b) {
    if (a.order != b.order) {
...
std::string SerializeFrontendActionMap(const FrontendActionMap& action_map) {
  ...
  std::sort(sorted_actions.begin(), sorted_actions.end(), [](const FrontendAction& a, const FrontendAction& b) {
    if (a.order != b.order) {
      return a.order < b.order;
    }
    return a.key < b.key;
