...
namespace uam
{
	namespace
	{
		...
		std::string EscapeValue(const std::string& value)
		{
			...
			const bool needs_quotes = value.find_first_of(" \t\r\n#;=,\"'") != std::string::npos || value.front() == ' ' || value.back() == ' ';
		...
		ProviderProfile FinalizeProfile(ProviderProfile profile)
		{
			...
			profile.runtime_flags.erase(std::remove_if(profile.runtime_flags.begin(), profile.runtime_flags.end(), [](const std::string& flag) { return Trim(flag).empty(); }), profile.runtime_flags.end());
		...
		std::vector<ProviderProfile> ParseProfilesText(const std::string& text, std::string* error_out)
		{
			...
			{
				if (!line.empty() && static_cast<unsigned char>(line[0]) == 0xEF && line.size() >= 3 && static_cast<unsigned char>(line[1]) == 0xBB && static_cast<unsigned char>(line[2]) == 0xBF)
				{
					line = line.substr(3);
				}
				...
				{
					const std::vector<std::string> parsed_flags = (key == "runtime_flags") ? SplitListValue(value) : std::vector<std::string>{value};
	...
	{
		return ProviderProfile{"gemini", "Gemini CLI", "gemini {resume} {flags} {prompt}", true, {}};
	}
	...
	std::vector<ProviderProfile> ProviderProfileStore::LoadUserProfiles(const std::filesystem::path& file_path, std::string* error_out)
	{
		if (error_out != nullptr)
		{
			error_out->clear();
	...
	bool ProviderProfileStore::SaveUserProfiles(const std::filesystem::path& file_path, const std::vector<ProviderProfile>& profiles, std::string* error_out)
	{
		if (error_out != nullptr)
		{
			error_out->clear();
	...
	std::vector<ProviderProfile> ProviderProfileStore::MergeProfiles(const std::vector<ProviderProfile>& base_profiles, const std::vector<ProviderProfile>& overlay_profiles)
	{
		...
		for (const ProviderProfile& overlay : overlay_profiles)
		{
			...
			auto it = std::find_if(merged.begin(), merged.end(), [&](const ProviderProfile& profile) { return Trim(profile.id) == id; });
	...
	std::vector<ProviderProfile> ProviderProfileStore::LoadProfiles(const std::filesystem::path& data_root, std::string* error_out)
	{
		if (error_out != nullptr)
		{
			error_out->clear();
	...
	bool ProviderProfileStore::SaveProfiles(const std::filesystem::path& data_root, const std::vector<ProviderProfile>& profiles, std::string* error_out)
	{
		return SaveUserProfiles(ProfilesFilePath(data_root), profiles, error_out);
	}

	const ProviderProfile* ProviderProfileStore::FindById(const std::vector<ProviderProfile>& profiles, const std::string& provider_id)
	{
		...
		const auto it = std::find_if(profiles.begin(), profiles.end(), [&](const ProviderProfile& profile) { return Trim(profile.id) == needle; });
