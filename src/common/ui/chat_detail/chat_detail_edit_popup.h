...
static void DrawEditUserMessagePopup(AppState& app, ChatSession& chat) {
  ...
  if (BeginCenteredPopupModal("Edit Message###edit_user_message_popup", nullptr, ImGuiWindowFlags_AlwaysAutoResize)) {
    ...
    if (!valid_target) {
      ...
      ImGui::TextColored(ui::kTextPrimary, "Edit Message");
    ...
    if (valid_target) {
      if (DrawButton("Apply and Continue", ImVec2(168.0f, 32.0f), ButtonKind::Primary)) {
