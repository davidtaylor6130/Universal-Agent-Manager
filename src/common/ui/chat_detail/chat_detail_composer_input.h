...
static void DrawInputContainer(AppState& app, ChatSession& chat) {
  ...
  if (ImGui::Checkbox("RAG", &rag_enabled_for_chat)) {
    ...
    SaveAndUpdateStatus(
        app, chat, chat.rag_enabled ? "RAG enabled for this chat." : "RAG disabled for this chat.", "Failed to save chat RAG setting.");
  }
  ImGui::SameLine();
  ...
  if (ImGui::BeginPopup("composer_rag_sources_popup")) {
    ...
    std::sort(candidate_folders.begin(), candidate_folders.end(), [](const fs::path& lhs, const fs::path& rhs) {
