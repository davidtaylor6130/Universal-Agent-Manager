...
inline bool DrawMiniIconGlyph(ImDrawList* draw, const ImVec2& min, const ImVec2& max, const char* glyph, const ImU32 color)
{
	...
	if (std::strcmp(glyph, "icon:new_chat") == 0)
	{
		...
		draw->AddTriangleFilled(ImVec2(bubble_min.x + 1.6f * icon_scale, bubble_max.y), ImVec2(bubble_min.x + 4.0f * icon_scale, bubble_max.y), ImVec2(bubble_min.x + 2.0f * icon_scale, bubble_max.y + 2.2f * icon_scale), color);
...
inline bool DrawMiniIconButton(const char* id, const char* glyph, const ImVec2& size = ImVec2(22.0f, 22.0f), const bool danger = false)
{
	...
	const ImVec4 bg = hovered ? (danger ? (light ? Rgb(220, 38, 38, 0.16f) : Rgb(255, 107, 107, 0.16f)) : (light ? Rgb(9, 31, 63, 0.09f) : Rgb(255, 255, 255, 0.10f))) : ui::kTransparent;
	...
	if (!DrawMiniIconGlyph(draw, min, max, glyph, icon_color))
	{
		...
		draw->AddText(ImVec2(min.x + (scaled_size.x - text_size.x) * 0.5f, min.y + (scaled_size.y - text_size.y) * 0.5f), icon_color, glyph);
	}
