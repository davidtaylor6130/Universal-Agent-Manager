...
/// </summary>
static bool BrowsePathWithNativeDialogWindows(const PathBrowseTarget target,
                                              const fs::path& initial_path,
                                              std::string* selected_path_out,
  ...
  if (!initial_folder.empty()) {
    ...
    if (SUCCEEDED(SHCreateItemFromParsingName(folder_wide.c_str(), nullptr, IID_PPV_ARGS(&folder_item))) &&
        folder_item != nullptr) {
...
/// </summary>
static bool BrowsePathWithNativeDialog(const PathBrowseTarget target,
  ...
  std::string script =
      "set selectedPath to POSIX path of (choose " +
      std::string(target == PathBrowseTarget::Directory ? "folder" : "file") + " with prompt " +
      std::string(target == PathBrowseTarget::Directory ? "\"Select folder\"" : "\"Select file\"");
  if (!initial_path.empty()) {
  ...
  *selected_path_out = selected_path;
  ...
  std::string command;
  if (has_zenity) {
    command = "zenity --file-selection --title=" +
              ShellQuotePath(target == PathBrowseTarget::Directory ? "Select folder" : "Select file");
    if (target == PathBrowseTarget::Directory) {
      command += " --directory";
    }
    if (!initial_path.empty()) {
      std::string filename_hint = initial_path.string();
      if (target == PathBrowseTarget::Directory && !filename_hint.empty() &&
          filename_hint.back() != '/' && filename_hint.back() != '\\') {
        filename_hint.push_back('/');
      }
      command += " --filename=" + ShellQuotePath(filename_hint);
    }
  } else {
    command = std::string("kdialog ") +
              (target == PathBrowseTarget::Directory ? "--getexistingdirectory" : "--getopenfilename");
    if (!initial_path.empty()) {
...
/// </summary>
static bool DrawPathInputWithBrowseButton(const char* label,
                                          std::string& value,
                                          const char* browse_button_id,
                                          const PathBrowseTarget target,
                                          const float total_width = -1.0f,
                                          bool* input_deactivated_after_edit_out = nullptr,
                                          bool* picked_with_dialog_out = nullptr,
                                          std::string* error_out = nullptr) {
