...
static void DrawRuntimeModelSelectionModal(AppState& app)
{
	...
	const bool folder_changed = DrawPathInputWithBrowseButton("##runtime_model_folder_directory", app.settings.models_folder_directory, "runtime_model_folder_picker", PathBrowseTarget::Directory, -1.0f, &input_deactivated_after_edit, &picked_with_dialog, &browse_error);
	...
	const std::string combo_preview = has_models ? (app.runtime_model_selection_id.empty() ? std::string("(select model)") : app.runtime_model_selection_id) : std::string("(no models found)");
