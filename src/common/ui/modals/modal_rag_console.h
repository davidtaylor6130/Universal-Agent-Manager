...
static void DrawRagConsoleModal(AppState& app) {
  ...
  ImGui::InputText("##rag_console_source_directory", &app.settings.rag_project_source_directory);
  ...
  if (scan_state.lifecycle == RagScanLifecycleState::Running) {
    if (scan_state.total_files > 0) {
      const float progress = std::clamp(
          static_cast<float>(scan_state.files_processed) / static_cast<float>(scan_state.total_files), 0.0f, 1.0f);
