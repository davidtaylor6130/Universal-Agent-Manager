...
class RagIndexService
{
  public:
	...
	std::vector<RagSnippet> RetrieveTopK(const std::filesystem::path& workspace_root, const std::string& query);
	/// <summary>Retrieves snippets with explicit max/min limits.</summary>
	std::vector<RagSnippet> Retrieve(const std::filesystem::path& workspace_root, const std::string& query, std::size_t max_results, std::size_t min_results, std::string* error_out = nullptr);
