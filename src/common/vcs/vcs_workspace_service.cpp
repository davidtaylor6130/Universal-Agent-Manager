...
std::string BuildWorkingDirectoryCommand(const std::filesystem::path& working_directory,
...
VcsCommandResult RunCommand(const std::filesystem::path& working_directory,
                            const std::string& command,
                            const int timeout_ms,
                            const std::size_t output_limit_bytes) {
  if (working_directory.empty()) {
    return VcsCommandResult{false, false, false, -1, "", "Workspace directory is empty."};
  }

  const std::string full_command =
      BuildWorkingDirectoryCommand(working_directory, command) + " 2>&1";

  std::promise<VcsCommandResult> promise;
  std::future<VcsCommandResult> future = promise.get_future();
  std::thread([full_command, output_limit_bytes, promise = std::move(promise)]() mutable {
...
VcsCommandResult VcsWorkspaceService::ReadSnapshot(const std::filesystem::path& workspace_root,
